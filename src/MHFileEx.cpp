...
BOOL CMHFileEx::OpenTxt( const char* fullfilename )
{
	...
	if( m_fp )
	{
		...
		while( !feof(m_fp) )
		{
			m_pData[m_Header.dwDataSize] = static_cast<char>(fgetc( m_fp ));
			m_Header.dwDataSize++;
		...
		*/
		...
		if(m_AESFile.IsExistKey())
		{
			...
			*/
			...
			{
				wchar_t str[512];
				wsprintfW(str, L"File Open Error : %s",fullfilename);
...
BOOL CMHFileEx::SaveToTxtOld( const char* filename )
{
...
bool CopyAndModifyExtension(const char* szFullFileName,
	const char* szNewExtension,
	char* szOutputBuffer,
	size_t outputBufferSize) 
{
	// Copy the original filename
	strncpy(szOutputBuffer, szFullFileName, outputBufferSize - 1);
	szOutputBuffer[outputBufferSize - 1] = '\0';

	// Find the last dot
	char* pLastDot = strrchr(szOutputBuffer, '.');

	if (pLastDot != nullptr) 
	{
		// Replace extension
		size_t baseLength = pLastDot - szOutputBuffer;
		if (baseLength + strlen(szNewExtension) + 1 <= outputBufferSize)
		{
			strcpy(pLastDot + 1, szNewExtension);
			return true;
		}
	}
	else
	{
		// Append extension
		size_t currentLen = strlen(szOutputBuffer);
		if (currentLen + 1 + strlen(szNewExtension) + 1 <= outputBufferSize) 
		{
			strcat(szOutputBuffer, ".");
			strcat(szOutputBuffer, szNewExtension);
			return true;
		}
	}
...
BOOL CMHFileEx::SaveToTxt(const char* newExtension)
{
	MakeFullFileName();
	...
	{
		if (!CopyAndModifyExtension(m_szFullFileName, newExtension, szTxtFileName, 256))
		{
	...
	{
		strcpy(szTxtFileName, m_szFullFileName);
	//	strcat(szTxtFileName, ".");
	//	strcat(szTxtFileName, m_szExt);
	}

	m_fp = fopen(szTxtFileName, "wb" );
...
// 20090515 ONS AES알고리즘을 사용하여 bin파일을 암호화한다(START)
			...
			if (bAES)
			{
				...
				{
					MessageBox( NULL, L"Failed Save To Bin", NULL, MB_OK );
					return FALSE;
				}
			}
			else
...
BOOL CMHFileEx::SaveToBin(bool bAES)
{
	MakeFullFileName();
	return SaveToBin( m_szFullFileName, bAES );
}
...
BOOL CMHFileEx::CheckHeader()
{
	...
	{
		Clear();
		MessageBox( NULL, L"Header Version Error!!", L"Error!!", MB_OK );
		return FALSE;
	}
	if( m_Header.dwType == 0 )
	{
		Clear();
		MessageBox( NULL, L"Header Type Error!!", L"Error!!", MB_OK );
		return FALSE;
	}
	if( m_Header.dwDataSize > MEGA )
	{
		Clear();
		MessageBox( NULL, L"Header DataSize Error!!", L"Error!!", MB_OK );
		return FALSE;
	}
...
BOOL CMHFileEx::CheckCRC()
{
	...
	{
		Clear();
		MessageBox( NULL, L"CheckCrc Error!!", L"Error!!", MB_OK );
		return FALSE;
	}

	// decode
	char crc = static_cast<char>(m_Header.dwType);
	for( DWORD i = 0; i < m_Header.dwDataSize; ++i )
	{
		crc = crc + m_pData[i];
		m_pData[i] = m_pData[i] - (char)i;
		if( i%m_Header.dwType == 0 )
			m_pData[i] = m_pData[i] - static_cast<char>(m_Header.dwType);
	}
...
BOOL CMHFileEx::ConvertBin()
{
	...
	for( DWORD i = 0; i < m_Header.dwDataSize; ++i )
	{
		m_pBinData[i] = m_pBinData[i] + static_cast<char>(i);
		if( i%m_Header.dwType == 0 )
			m_pBinData[i] = m_pBinData[i] + static_cast<char>(m_Header.dwType);
...
BOOL CMHFileEx::InitFileName( const char* fullfilename )
{
	...
	int len = static_cast<int>(strlen(m_szFullFileName));
	int i=0, j=0, k=0;
...
{
	int len = static_cast<int>(strlen(m_szFullFileName));
	int lenext = static_cast<int>(strlen(m_szExt));
	int i=0, j=0;
...
void CMHFileEx::SetData( char* str )
{
	if( m_pData )
	{
		memcpy( m_pData, str, strlen( str ));
		m_Header.dwDataSize = static_cast<DWORD>(strlen( str ));
	}
}
...
{
	int len = static_cast<int>(strlen(m_szFullFileName));
	int lenext = static_cast<int>(strlen(m_szDOF));
	int i=0, j=0;
...
//	m_pBinData[m_DSOFHeader.dwDataSize] = 0;
	...
	{
		m_pBinData[i] = (char)(m_pBinData[i] + (i + m_DOFHeader.dwSeed + m_DOFTail.dwSeed));
		m_crc1 = m_crc1 + m_pBinData[i];
	}
...
BOOL CMHFileEx::CheckDOF()
{
	...
	{
		Clear();
		MessageBox( NULL, L"Header Seed Error!!", L"Error!!", MB_OK );
		return FALSE;
	}
	...
	{
		crc = crc + m_pData[i];
		m_pData[i] = (char)(m_pData[i] - (i + m_DOFHeader.dwSeed + m_DOFTail.dwSeed));
	}
	if( m_crc1 != crc )
	{
		Clear();
