...
public:
	...
	BOOL	SaveToTxtOld( const char* filename );
	BOOL	SaveToTxt(const char* newExtension="");
	BOOL	SaveToBin( const char* filename, bool bAES = TRUE );
	BOOL	SaveToBin(bool bAES = TRUE);
	BOOL	CheckHeader();
	BOOL	CheckCRC();
	BOOL	ConvertBin();
